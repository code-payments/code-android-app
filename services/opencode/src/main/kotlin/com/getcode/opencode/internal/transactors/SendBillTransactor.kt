...
internal class SendBillTransactor(
    ...
    fun with(amount: LocalFiat, owner: AccountCluster): List<Byte> {
        ...
        return OpenCodePayload(
            ...
            println("rendezvous=${it.rendezvous.getPublicKeyBase58()}")
        }.codeData.toList()
    }

    suspend fun start(): Result<TransactionMetadata.SendPublicPayment> {
        val ownerKey = owner
            ?: return Result.failure(SendTransactorError.Other(message = "No owner key. Did you call with() first?"))
        val rendezvous = rendezvousKey
            ?: return Result.failure(SendTransactorError.Other(message = "No rendezvous key. Did you call with() first?"))
        val transferRequest = messagingController.awaitRequestToGrabBill(scope, rendezvous)
            ?: return Result.failure(SendTransactorError.Other(message = "No message received"))
        ...
        if (!isValid) {
            return Result.failure(SendTransactorError.DestinationSignatureInvalidException())
        }
        ...
        if (receivingAccount == transferRequest.account) {
            ...
            // of this function.
            return Result.failure(SendTransactorError.DuplicateTransferException())
        }
    ...
    sealed class SendTransactorError(
        ...
        class DuplicateTransferException : SendTransactorError()
        class DestinationSignatureInvalidException : SendTransactorError()
        data class Other(
            override val message: String? = null,
            override val cause: Throwable? = null
        ) : SendTransactorError()
