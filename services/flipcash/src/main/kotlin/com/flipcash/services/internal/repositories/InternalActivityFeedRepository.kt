...
internal class InternalActivityFeedRepository(
    ...
    override suspend fun getLatestNotifications(
        ...
        maxItems: Int
    ): Result<List<ActivityFeedNotification>> = service.getLatestNotifications(owner, type, maxItems)
    ...
    ): Result<List<ActivityFeedNotification>> = service.getNotificationsPage(owner, type, queryOptions)
        .onFailure { ErrorUtils.handleError(it) }
        .map { items -> items.map { mapper.map(it) } }
    ...
    override suspend fun getBatchedNotifications(
        owner: Ed25519.KeyPair,
        ids: List<ID>
    ): Result<List<ActivityFeedNotification>> = service.getNotificationsByIds(owner, ids)
    .onFailure { ErrorUtils.handleError(it) }
    .map { items -> items.map { mapper.map(it) } }
}