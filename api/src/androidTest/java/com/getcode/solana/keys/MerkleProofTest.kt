...
class MerkleProofServerTest {
    ...
    @Test
    fun testValidProof() {
        val originalCommitment = "leaf0".toUTF8Bytes()
        assert(originalCommitment.verifyContained(root, proof))
    }

    @Test
    fun testInvalidCommitment() {
        val originalCommitment = "leaf1".toUTF8Bytes()
        assert(!originalCommitment.verifyContained(root, proof))
    }

    @Test
    fun testSingleInvalidNode() {
        val originalCommitment = "leaf0".toUTF8Bytes()

        proof.forEachIndexed { index, _ ->
            val modifiedProof = proof.toMutableList()
...
class MerkleProofClientTests {
    ...
    @Test
    fun testValidProof() {
        assert(leaf.verifyContained(root,proof))
    }

    @Test
    fun testInvalidProof() {
        val modifiedBytes = leaf.bytes.toMutableList()
        modifiedBytes[31] = 0xFF.toByte()
        val modifiedLeaf  = PublicKey(modifiedBytes)

        assert(!modifiedLeaf.verifyContained(root, proof))
    }
    ...
    fun testSingleInvalidNode() {
        proof.forEachIndexed { index, _ ->
            ...
            val nodeBytes = modifiedProof[index].bytes.toMutableList()
            nodeBytes[31] = 0xFF.toByte() // Modify the last byte of every node
            modifiedProof[index] = PublicKey(nodeBytes)

            assert(!leaf.verifyContained(root, modifiedProof))
        }
    }
