...
ReedSolomonDecoder::ReedSolomonDecoder(const Ref<GenericGF>& field_) : field(field_) {}

ReedSolomonDecoder::~ReedSolomonDecoder() = default;
...
void ReedSolomonDecoder::decode(ArrayRef<int> received, int twoS) {
  ...
  const Ref<GenericGFPoly>& sigma = sigmaOmega[0];
  const Ref<GenericGFPoly>& omega = sigmaOmega[1];
...
ArrayRef<int> ReedSolomonDecoder::findErrorLocations(const Ref<GenericGFPoly>& errorLocator) {
...
ArrayRef<int> ReedSolomonDecoder::findErrorMagnitudes(const Ref<GenericGFPoly>& errorEvaluator, ArrayRef<int> errorLocations) {
