...
private:
  Ref<GenericGF> field;
public:
  ReedSolomonDecoder(const Ref<GenericGF>& fld);
  ~ReedSolomonDecoder();
...
private:
  ArrayRef<int> findErrorLocations(const Ref<GenericGFPoly>& errorLocator);
  ArrayRef<int> findErrorMagnitudes(const Ref<GenericGFPoly>& errorEvaluator, ArrayRef<int> errorLocations);
};
}
