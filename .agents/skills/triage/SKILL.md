...
bash .agents/skills/triage/scripts/bugsnag-top.sh --url "https://app.bugsnag.com/org/project/errors/abc123"

# Specific issue by error ID
bash .agents/skills/triage/scripts/bugsnag-top.sh --error-id abc123
```

Otherwise, fetch the top open issue automatically:

```bash
bash .agents/skills/triage/scripts/bugsnag-top.sh
```
...
`.agents/skills/triage/references/event-shape.md`.
...
2. **Download the R8 mapping** using the `r8-mapping` skill:
   ```bash
   bash .agents/skills/r8-mapping/scripts/r8-mapping.sh <versionCode>
   ```
...
```bash
bash .agents/skills/build-lookup/scripts/build-lookup.sh <versionCode>
```
...
Use the template in `.agents/skills/triage/references/brief-template.md`.

Save the brief to `.agents/plans/triage-<error_id>.md`.
