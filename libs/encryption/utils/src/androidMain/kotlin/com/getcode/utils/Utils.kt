...
object Utils {
    ...
    /** Reads an unsigned 16-bit little-endian integer from [bytes] at [offset]. */
    @JvmStatic
    ...
    /** Reads an unsigned 32-bit little-endian integer from [bytes] at [offset]. */
    @JvmStatic
    ...
    /** Reads an unsigned 32-bit big-endian integer from [bytes] at [offset]. */
    @JvmStatic
    ...
    /** Reads a signed 64-bit little-endian integer from [bytes] at [offset]. */
    @JvmStatic
    ...
    /** Writes a signed 64-bit integer to [out] at [offset] in little-endian format. */
    @JvmStatic
    ...
    /** Writes a uint16 LE integer to an [OutputStream]. */
    @JvmStatic
    ...
    /** Writes a uint32 LE integer to an [OutputStream]. */
    @JvmStatic
    @Throws(IOException::class)
    ...
    /** Writes a signed 64-bit LE integer to an [OutputStream]. */
    @JvmStatic
    @Throws(IOException::class)
    ...
    /** Reads bytes from [stream] until [count] bytes have been read or an EOF occurs. */
    @JvmStatic
    @Throws(IOException::class)
    fun readBytesFromStream(stream: InputStream, count: Int): ByteArray {
        val buffer = ByteArray(minOf(count, MAX_INITIAL_ARRAY_LENGTH))
        var offset = 0
        var remaining = count
        while (remaining > 0) {
            val read = stream.read(buffer, offset, remaining)
            if (read < 0) throw IOException("Tried to read $count bytes, but only $offset bytes available before EOF")
            remaining -= read
            offset += read
        }
        return buffer
    ...
    init {
        ...
        runtime = when {
            ...
            else -> { Timber.i("Unknown java.runtime.name '$runtimeProp'"); null }
        }
        ...
        os = when {
            ...
            else -> { Timber.i("Unknown os.name '$osProp'"); null }
        }
    }
