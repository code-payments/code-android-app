...
class TimelockProgram_CloseAccounts_Test {

    @Test
    fun testDecode() {
        val (transaction, _) = SolanaTransaction.mockCloseDormantAccount()
        val rawInstruction = transaction.message.instructions[5]
        val instruction = TimelockProgram_CloseAccounts.newInstance(rawInstruction)
