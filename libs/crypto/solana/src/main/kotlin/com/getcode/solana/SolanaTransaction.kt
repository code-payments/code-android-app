...
data class SolanaTransaction(val message: Message, val signatures: List<com.getcode.solana.keys.Signature>) {
    val identifier
    ...
    fun sign(vararg keyPairs: Ed25519.KeyPair): List<com.getcode.solana.keys.Signature> {
        ...
        val newSignatures = mutableListOf<com.getcode.solana.keys.Signature>()
        ...
        keyPairs.forEach { keyPair ->
            ...
            newSignatures.add(com.getcode.solana.keys.Signature(signature.toList()))
        }
    ...
    inline fun <reified T> findInstruction(cb: (instruction: Instruction) -> com.getcode.solana.instructions.InstructionType): T? {
    ...
    companion object {
        fun fromList(list: List<Byte>): SolanaTransaction? {
            ...
            val signatures: List<com.getcode.solana.keys.Signature> =
        ...
        fun newInstance(
            payer: com.getcode.solana.keys.PublicKey,
            recentBlockhash: Hash?,
            instructions: List<Instruction>
        ): SolanaTransaction {
            val accounts = mutableListOf<com.getcode.solana.keys.AccountMeta>()
            accounts.add(com.getcode.solana.keys.AccountMeta.payer(publicKey = payer))

            instructions.forEach {
                accounts.add(com.getcode.solana.keys.AccountMeta.program(publicKey = it.program))
            ...
            val signatures = mutableListOf<com.getcode.solana.keys.Signature>()
                .apply {
                    for (i in 0 until message.header.requiredSignatures) {
                        add(com.getcode.solana.keys.Signature.zero)
                    }
                }
