...
import com.flipcash.app.tipping.internal.FindByUsernameViewModel
...
fun FindByUsernameScreen() {
    ...
    val viewModel = hiltViewModel<FindByUsernameViewModel>()
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        FindByUsernameScreenContent(state, viewModel::dispatchEvent)
    }
    ...
    LaunchedEffect(viewModel, navigator) {
        viewModel.eventFlow
            ...
            .onEach { resolved ->
                // Not `replace`: that drops this screen only, and the chooser underneath it would
                // stay between the chat and the Chats list. Both are entry UI for the chat about to
                // open, and popping by route type leaves the pop correct if either is absent —
                // reaching this screen directly, say from a deeplink.
                navigator.popUntil {
...
@Composable
private fun ColumnScope.FindByUsernameScreenContent(
    state: FindByUsernameViewModel.State,
    dispatchEvent: (FindByUsernameViewModel.Event) -> Unit,
) {
    val keyboard = rememberKeyboardController()
    CodeScaffold(
        modifier = Modifier
            ...
            CodeButton(
                ...
                onClick = {
                    keyboard.hideIfVisible {
                        dispatchEvent(FindByUsernameViewModel.Event.LookupUsername)
                    }
                },
            )
        }
